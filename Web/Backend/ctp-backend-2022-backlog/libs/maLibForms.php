...
function mkInput($type, $name, $value="", $misc=[]) {  
  ...
  // On produit une info-bulle, si définie dans misc
  $infobulle = "";
  if (isset($misc["infobulle"])) {
    $infobulle = " title=\"$misc[infobulle]\"";
  }
  ...
  $res = "<input$id type=\"$type\" name=\"$name\" value=\"$value\"$selectionne$infobulle/>\n";
...
// TODO
// Fonction pour créer une div selon un élément
function mkDiv($text,$css){
  echo "<div class=\"" . $css . "\">" . $text . "</div>"; 
