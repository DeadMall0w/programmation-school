...
do
{
 ...
 // Affichage d'un menu
    ...
    printf("1 - Generer un tableau de maniere ordonnée \n");
    ...
    switch (choix) 
    {
        case 1 : //Saisie de la dimension effective du tableau
                ...
                geneord(T1,dernier);
                 break;
        case 2 : printf(" RECHERCHE DANS UN TABLEAU TRIE \n");
                ...
                break; 
        case 3 : printf(" AJOUT DANS UN TABLEAU TRIE \n");
                ...
                printf("L'ajout d'un element dans un tableau de %d elements a pris %ld us !!! \n",dernier-1,duree); 
                 break;
        case 4 : printf(" MODIFICATION DANS UN TABLEAU TRIE \n");
                ...
                break;            
        case 5 : printf(" SUPPRESSION DANS UN TABLEAU TRIE \n");
...
int rechercher(element T[], const int taille, const element valeur)
{
    ...
    while (min <= max) {
        ...
        if (T[middle] == valeur) {
            return middle; // Retourne l'indice où `valeur` est trouvé
        }
        
        if (T[middle] > valeur) {
            max = middle - 1; // Recherche dans la partie gauche
        } else {
            min = middle + 1; // Recherche dans la partie droite
        }
    }
...
int ajouter(element T[], int *taille, element valeur, int taillemax)
{
    ...
    if (*taille >= taillemax) {
        return 0; // L'ajout est impossible
    }

    // Cas ou l'élément doit etre ajouté à la fin
    if (valeur >= T[*taille]) {
        T[*taille + 1] = valeur;
        (*taille)++;
        return *taille; // Retourne la position d'insertion
    }
    
    // Cas ou l'element doit etre ajouté au début
    if (valeur <= T[1]) {
        ...
        T[1] = valeur;
        (*taille)++;
        return 1; // Position d'insertion
    }

    // Cas general : recherche de la position d'insertion
    int min = 1, max = *taille, middle;

    while (min <= max) {
        middle = (min + max) / 2;

        if (T[middle] == valeur) { // l'element existe deja
            min = middle;
            break;
        }
    ...
    // `min` est maintenant la position d'insertion
    for (int i = *taille + 1; i > min; i--) {
        T[i] = T[i - 1];
    }
    T[min] = valeur;
    (*taille)++;

    return min; // Retourne la position d'insertion  
}
...
element modifier(element T[], const int taille, const int pos, const element valeur)
{
    ...
    int n=taille;
    T[0] = supprimer(T, &n, pos);
    if(T[0] == 0){ // L'élement n'a pas été supprimé
        return 0.0;
    }
    ajouter(T, &n, valeur, MAXCOMP+1);
    return T[0];
}
